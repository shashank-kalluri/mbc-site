...
export default function Videos() {
  return (
    <section className="bg-[#1A2A36] py-16 sm:py-20">
      <div className="max-w-screen-xl mx-auto px-6 sm:px-10 lg:px-16">
        ...
        <div className="flex items-end justify-between mb-10 gap-6">
          ...
          <p className="text-white/30 text-sm hidden sm:block pb-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
          {episodes.map(({ id, episode, title, comingSoon }) => (
            <div key={episode} className="flex flex-col gap-3">
              {comingSoon ? (
                <div className="relative w-full rounded-2xl overflow-hidden bg-[#293C4B] border border-white/5" style={{ paddingTop: "56.25%" }}>
                ...
                <div className="relative w-full rounded-2xl overflow-hidden shadow-xl shadow-black/40" style={{ paddingTop: "56.25%" }}>
                  <iframe
                    src={`https://drive.google.com/file/d/${id}/preview`}
                    className="absolute w-full"
                    style={{ top: "-40%", height: "140%" }}
              ...
              <div>
                ...
                <p className="text-white/70 text-sm font-medium mt-0.5">{title}</p>
              </div>
            </div>
          ))}
        </div>
